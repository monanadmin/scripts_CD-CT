...
export PATH=$NETCDF/bin:$NETCDF_C/bin:$PATH
echo "netcdf=$NETCDF"
...
rm -rf ${MONANDIR}/src/core_atmosphere/inc/ ${MONANDIR}/src/core_init_atmosphere/inc/
rm -rf ${MONANDIR}/default_inputs
rm ${MONANDIR}/src/lib*.a ${MONANDIR}/*.TBL
rm ${MONANDIR}/stream* ${MONANDIR}/namelist.* 
rm ${MONANDIR}/make_*output.atmosphere ${MONANDIR}/make_*output.init_atmosphere
DATE_TIME_NOW=\$(date +"%Y%m%d%H%M%S")

#sed -i "s/DMPAS_USE_MPI_F08/DMPAS_USE_MPI_H/" ${MONANDIR}/Makefile
#sed -i "s,framework.*$,framework -I../../../../external/esmf_time_f90," ${MONANDIR}/src/core_atmosphere/physics/physics_noahmp/src/Makefile
...
make -j 8 pgi CORE=atmosphere USE_PIO=false OPENACC=true OPENMP=true PRECISION=single 2>&1 | tee \${MAKE_OUT_FILE}
...
make -j 8 pgi CORE=init_atmosphere USE_PIO=false OPENACC=true OPENMP=true PRECISION=single 2>&1 | tee \${MAKE_OUT_FILE}
...
cp /scratch/cptec/bin/ungrib.exe ${EXECS}
