...
SCRIPTS=${DIRHOMES}/scripts.sdumont;   mkdir -p ${SCRIPTS}
...
rm -f ${SCRIPTS}/submit_atm_cptec.srm 
cat << EOF0 > ${SCRIPTS}/submit_atm_cptec.srm 
#!/bin/bash
...
#checar variavel de ambiente do cuda  para gerar log 
...
# mudanca do namelist.atmosphere pois a suite do monan nao funciona com o compilador pgi
sed -i "s/mesoscale_reference_monan/mesoscale_reference/" ${SCRIPTS}/namelist.atmosphere

date
time mpirun -n \$SLURM_NTASKS ./atmosphere_model
date
...
EOF0
chmod a+x ${SCRIPTS}/submit_atm_cptec.srm
...
echo -e  "sbatch ${SCRIPTS}/submit_atm_cptec.srm"
#sbatch --wait -N2 -n96 -c2 ${SCRIPTS}/submit_atm_cptec.srm
#sbatch --wait -N1 -n48 -c2 ${SCRIPTS}/submit_atm_cptec.srm
#sbatch --wait -N1 -n16 -c2 ${SCRIPTS}/submit_atm_cptec.srm
#sbatch --wait -N1 -n16 -c1 ${SCRIPTS}/submit_atm_cptec.srm
#sbatch --wait ${SCRIPTS}/submit_atm_cptec.srm
sbatch --wait -N1 -n${cores} -c2 ${SCRIPTS}/submit_atm_cptec.srm
mv ${SCRIPTS}/submit_atm_cptec.srm ${DATAOUT}/${YYYYMMDDHHi}/Model/logs
...
. /scratch/cptec/.spack/v0.23.1/env_spack.sh
. $(pwd)/setenv.bash
...
date
time mpirun -n \${SLURM_NTASKS} ./\${executable}
date
